defmodule FraytWave.MCP.ToolsTest do
  use ExUnit.Case, async: true

  test "tools have valid callbacks" do
    {_, dispatch_map} = FraytWave.MCP.Server.tools_and_dispatch()
  ...
  test "tools are hidden from the list when excluded" do
    ...
    assert Enum.any?(
             FraytWave.MCP.Server.tools({%{}, %{exclude_tools: [], include_tools: nil}}),
             &(&1.name == "project_eval")
           )
  ...
  test "fs tools have listable callback" do
    tools = FraytWave.MCP.Tools.FS.tools()
